...
def stance_proxy_score(
    tokens: list[str], role: str, pro_words: set[str], con_words: set[str]
) -> float:
...
def compute_interaction_features(df: pd.DataFrame, stop_words: set[str]) -> pd.DataFrame:
    ...
    for _, row in out.iterrows():
        ...
        lexical_prev_other.append(
            lexical_overlap(lemmas, current_prev_nonself_tokens or [], stop_words)
        )
    ...
    out["novelty_vs_prev_turn_lexical"] = 1.0 - out["lexical_overlap_prev_turn"].clip(
        lower=0.0, upper=1.0
    )
    out["novelty_vs_prev_turn_semantic"] = 1.0 - out["semantic_similarity_prev_turn"].clip(
        lower=-1.0, upper=1.0
    )
    out["novelty_vs_prev_opponent_turn_lexical"] = 1.0 - out[
        "lexical_overlap_prev_opponent_turn"
    ].clip(lower=0.0, upper=1.0)
    out["novelty_vs_prev_opponent_turn_semantic"] = 1.0 - out[
...
def build_stance_change_events(df: pd.DataFrame, group_cols: list[str]) -> pd.DataFrame:
    ...
    sub = (
        df[needed]
        .dropna(subset=["stance_proxy_score"])
        .sort_values(["run_id", "turn_index"])
        .copy()
    )
    if sub.empty:
        return pd.DataFrame()
...
def summarise_stance_change(events_df: pd.DataFrame, group_cols: list[str]) -> pd.DataFrame:
    if events_df.empty:
        return pd.DataFrame()
    out = events_df.groupby(group_cols, as_index=False).agg(
    ...
    out["stance_flip_rate"] = out.apply(
        lambda row: (
            0.0
            if float(row["debates"]) == 0
            else float(row["stance_flip_count"]) / float(row["debates"])
        ),
        axis=1,
    )
    return out
...
def summarise_turning_points(
    ...
    out["turning_point_rate"] = out.apply(
        lambda row: (
            0.0
            if float(row["observed_turns"]) == 0
            else float(row["turning_point_count"]) / float(row["observed_turns"])
        ),
        axis=1,
    )
    return out
