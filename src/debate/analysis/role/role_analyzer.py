...
def extract_nrc_emotions(text: str) -> dict[str, float]:
    ...
    counts = extract_nrc_emotion_counts(
        text,
        nrc_word_lexicon=EMOTION_LEXICON,
        emotions=EMOTION_LEXICON.keys(),
        unique_tokens=True,
    )
    return {f"nrc_emotion_{emotion}": count for emotion, count in counts.items()}
...
def load_and_enrich_data(
    ...
    console.print(
        f"[yellow]Enriched dataset not found at {output_csv}. Starting enrichment of {input_csv}...[/]"
    )
    ...
    if not skip_emotion:
        try:
            emotion_analyzer = EmotionAnalyzer.get_instance(model_name=emotion_model)
        except ImportError:
...
def analyze_moderator_dynamics(df: pd.DataFrame, output_dir: Path):
    ...
    neg_cols = [
        c
        for c in df.columns
        if c.endswith("_anger") or c.endswith("_fear") or c.endswith("_sadness")
    ]
    ...
    if not neg_cols:
        ...
        ax = sns.lineplot(
            data=turn_avg[turn_avg["speaker_role"] != "moderator"],
            x="turn_index",
            y="negative_score",
            hue="speaker_role",
            marker="o",
    ...
    if vocab_overlaps:
        ...
        overlaps_df["Phase"] = pd.Categorical(
            overlaps_df["Phase"], categories=["Early", "Middle", "Late"], ordered=True
        )
...
def analyze_persona_profiling(df: pd.DataFrame, output_dir: Path):
    ...
    if bert_cols:
        ...
        bert_melted = bert_means.melt(
            id_vars="speaker_role", var_name="Emotion", value_name="Average Score"
        )
    ...
    if (
        "strong_modality_score" in df.columns
        and "weak_modality_score" in df.columns
        and "word_count" in df.columns
    ):
        ...
        agents_df["strong_modality_density"] = (
            agents_df["strong_modality_score"] / valid_words
        ) * 100
        agents_df["weak_modality_density"] = (agents_df["weak_modality_score"] / valid_words) * 100

        mod_means = (
            agents_df.groupby("speaker_role")[["strong_modality_density", "weak_modality_density"]]
            .mean()
            .reset_index()
        )
        mod_means_melted = mod_means.melt(
            id_vars="speaker_role", var_name="Modality Type", value_name="Density (per 100 words)"
        )

        # Clean up labels for plotting
        mod_means_melted["Modality Type"] = (
            mod_means_melted["Modality Type"]
            .str.replace("_density", "")
            .str.replace("_", " ")
            .str.title()
        )
        ...
        plt.figure(figsize=(8, 6))
        ax = sns.barplot(
            data=mod_means_melted,
            y="speaker_role",
            x="Density (per 100 words)",
            hue="Modality Type",
            orient="h",
        )
        for container in ax.containers:
...
def analyze_tactic_asymmetry(df: pd.DataFrame, output_dir: Path):
    ...
    if "tactic_used" in agents_df.columns:
        ...
        tool_tactic_crosstab = (
            agents_df.groupby(["speaker_role", "tactic_used"])["used_search"].mean().reset_index()
        )
        tool_tactic_crosstab["used_search_pct"] = tool_tactic_crosstab["used_search"] * 100

        plt.figure(figsize=(10, 6))
        ax = sns.barplot(
            data=tool_tactic_crosstab, y="tactic_used", x="used_search_pct", hue="speaker_role"
        )
        for container in ax.containers:
        ...
        tool_counts = (
            agents_df.groupby(["round", "speaker_role", "used_search"]).size().unstack(fill_value=0)
        )
...
def analyze_interrogative_doubt(df: pd.DataFrame, output_dir: Path):
    ...
    if "question_count" in agents_df.columns and "subjectivity" in agents_df.columns:
        ...
        # Split Boxplot
        palette = {
            "Declarative (No Questions)": "#95a5a6",
            "Interrogative (Has Questions)": "#e74c3c",
        }
...
def analyze_epistemic_stubbornness(df: pd.DataFrame, output_dir: Path):
    ...
    if "stance" in agents_df.columns:
        ...
        if not stance_df.empty:
            summary = (
                stance_df.groupby("speaker_role")[["Maintained Stance", "Changed Stance"]].mean()
                * 100
            )

            ax = summary.plot(
                kind="bar", stacked=True, figsize=(8, 6), color=["#2ca02c", "#d9534f"]
            )
            for container in ax.containers:
                ax.bar_label(
    ...
    if "confidence" in agents_df.columns:
        ...
        for (did, role), group in agents_df.groupby(["debate_id", "speaker_role"]):
            ...
            if len(group) > 1:
                ...
                deltas.append({"speaker_role": role.title(), "Confidence Delta": delta})

        deltas_df = pd.DataFrame(deltas)
        if not deltas_df.empty:
            plt.figure(figsize=(8, 6))
            sns.violinplot(
                data=deltas_df, x="speaker_role", y="Confidence Delta", density_norm="width"
            )
            ...
            text_box = plt.text(
                ...
                linespacing=1.6,
                bbox=dict(
                    boxstyle="round,pad=0.5", facecolor="#f8f9fa", alpha=0.9, edgecolor="gray"
                ),
            )

            plt.savefig(
                output_dir / "6_2_stubbornness_index.svg",
                bbox_extra_artists=(
                    legend,
                    text_box,
                ),
...
def main(argv: list[str] | None = None):
    ...
    parser.add_argument(
        ...
        help="Directory containing the old artifacts",
    )
    parser.add_argument(
        "--output-root",
        type=str,
        default="results/analysis/role_analysis",
        help="Root directory for role analyzer outputs",
    )
    parser.add_argument(
        "--skip-emotion", action="store_true", help="Skip BERT emotion extraction (saves time)"
    )
    parser.add_argument(
        "--emotion-model",
        type=str,
        default=None,
        help="BERT emotion model name (defaults to config.ini)",
    )
    ...
    local_output_root = repo_root / Path(args.output_root)
    ...
    if not input_csv.exists():
        console.print(
            f"[red]Could not find {input_csv}. Ensure you're running from the project root.[/]"
        )
        return
