...
def load_nrc_emotion_lexicon(path: Path, nlp: Any) -> dict[str, set[str]]:
    path = Path(path)
    if not path.exists() or not path.is_file():
        raise FileNotFoundError(f"NRC lexicon file not found: {path}")

    lex: dict[str, set[str]] = {emotion: set() for emotion in DEFAULT_NRC_EMOTIONS}
    with open(path, encoding="utf-8") as handle:
        for raw_line in handle:
            ...
            if len(parts) < 3:
                continue
            word, emotion, assoc = parts[0], parts[1].lower(), parts[2]
            if emotion not in lex or str(assoc).strip() != "1":
                continue
            for var in _lemma_variants(nlp, _normalize_lex_token(word)):
                lex[emotion].add(var)
    return lex


def load_nlp_resources(nrc_path: Path, use_transformer_emotion_secondary: bool) -> NLPResources:
    ...
    nrc_lexicon = load_nrc_emotion_lexicon(nrc_path, nlp)
