...
def bootstrap_mean_ci(values: list[float], n_boot: int, ci: float = 0.95) -> tuple[float, float, float]:
    ...
    means = [float(np.random.choice(arr, size=len(arr), replace=True).mean()) for _ in range(n_boot)]
    alpha = (1 - ci) / 2
    return float(arr.mean()), float(np.quantile(means, alpha)), float(np.quantile(means, 1 - alpha))


def bootstrap_gap_ci(a_values: list[float], b_values: list[float], n_boot: int, ci: float = 0.95) -> tuple[float, float, float]:
...
def group_summary(df: pd.DataFrame, group_cols: list[str], metrics: list[str] | None = None) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame()
    ...
    for col in BINARY_RATE_COLS:
        if col in df.columns:
            rate = df.groupby(group_cols)[col].mean().reset_index().rename(columns={col: f"{col}_rate"})
            agg = agg.merge(rate, on=group_cols, how="left")

    total_words = (
        df.groupby(group_cols)["word_count"].sum().reset_index().rename(columns={"word_count": "total_words"})
...
def group_summary_bootstrap(df: pd.DataFrame, group_cols: list[str], metrics: list[str], n_boot: int) -> pd.DataFrame:
    rows = []
    if df.empty:
        return pd.DataFrame()
    for values, group in df.groupby(group_cols):
        ...
        for metric in metrics:
            if metric not in group.columns:
                continue
            mean_val, ci_low, ci_high = bootstrap_mean_ci(group[metric].dropna().tolist(), n_boot=n_boot)
            rows.append({**base, "metric": metric, "mean": mean_val, "ci_low": ci_low, "ci_high": ci_high, "n": int(group[metric].dropna().shape[0])})
    return pd.DataFrame(rows)


def trajectory_summary(df: pd.DataFrame, group_cols: list[str], step_col: str = "round") -> pd.DataFrame:
...
def winner_summary(df: pd.DataFrame, group_cols: list[str]) -> pd.DataFrame:
    ...
    winner_counts = debate_level.groupby(group_cols + ["winner_role"], as_index=False).size().rename(columns={"size": "debate_count"})
    totals = debate_level.groupby(group_cols, as_index=False).size().rename(columns={"size": "total_debates"})
    out = winner_counts.merge(totals, on=group_cols, how="left")
    out["winner_rate"] = out.apply(
        lambda row: 0.0 if float(row["total_debates"]) == 0 else float(row["debate_count"]) / float(row["total_debates"]),
        axis=1,
    )
    return out.sort_values(group_cols + ["winner_rate", "debate_count"], ascending=[True] * len(group_cols) + [False, False])


def role_contrast_summary(df: pd.DataFrame, metrics: list[str], n_boot: int, role_a: str = "proponent", role_b: str = "opponent", group_cols: list[str] | None = None) -> pd.DataFrame:
...
def paired_run_role_gap(df: pd.DataFrame, metrics: list[str], n_boot: int, role_a: str = "proponent", role_b: str = "opponent", group_cols: list[str] | None = None) -> pd.DataFrame:
    group_cols = group_cols or ["model_name"]
    ...
    for metric in metrics:
        ...
        if sub.empty:
            continue
        per_run = sub.groupby(group_cols + ["run_id", "debate_role"], as_index=False)[metric].mean(numeric_only=True)
        pivot = per_run.pivot_table(index=group_cols + ["run_id"], columns="debate_role", values=metric, aggfunc="mean")
...
def topic_role_balanced_comparison(df: pd.DataFrame, metrics: list[str], n_boot: int) -> pd.DataFrame:
...
def collect_error_cases(df: pd.DataFrame, examples_per_type: int = 20) -> pd.DataFrame:
    ...
    if {"word_count", "evidence_density_per_1k_words"}.issubset(df.columns):
        sub = df.sort_values(["word_count", "evidence_density_per_1k_words"], ascending=[False, True]).head(examples_per_type)
        for _, row in sub.iterrows():
            rows.append({"error_type": "long_but_evidence_poor", **row[["run_id", "model_name", "debate_role", "topic_id", "turn_index", "word_count", "utterance"]].to_dict()})

    if {"confidence", "evidence_density_per_1k_words"}.issubset(df.columns):
        sub = df.dropna(subset=["confidence"]).sort_values(["confidence", "evidence_density_per_1k_words"], ascending=[False, True]).head(examples_per_type)
        for _, row in sub.iterrows():
            rows.append({"error_type": "high_confidence_low_evidence", **row[["run_id", "model_name", "debate_role", "topic_id", "turn_index", "confidence", "utterance"]].to_dict()})

    if {"rebuttal_density_per_1k_words", "semantic_similarity_prev_opponent_turn"}.issubset(df.columns):
        sub = df.sort_values(["rebuttal_density_per_1k_words", "semantic_similarity_prev_opponent_turn"], ascending=[False, True]).head(examples_per_type)
        for _, row in sub.iterrows():
            rows.append({"error_type": "generic_rebuttal_marker", **row[["run_id", "model_name", "debate_role", "topic_id", "turn_index", "utterance"]].to_dict()})

    if {"winner_confidence", "winner_role"}.issubset(df.columns):
        sub = df[df["winner_confidence"].isin(["low", "medium"])].head(examples_per_type)
        for _, row in sub.iterrows():
            rows.append({"error_type": "ambiguous_winner_extraction", **row[["run_id", "model_name", "debate_role", "topic_id", "turn_index", "utterance"]].to_dict()})
...
def embedding_projection(all_df: pd.DataFrame, random_seed: int) -> pd.DataFrame:
    ...
    return valid[["run_id", "turn_index", "model_name", "debate_role", "topic_id", "stage", "embed_pca_1", "embed_pca_2"]]
...
def build_distinctive_terms_for_group(all_df: pd.DataFrame, mask: pd.Series, top_n: int, min_df: int) -> pd.DataFrame:
    ...
    other_mean = np.asarray(other_matrix.mean(axis=0)).ravel() if other_matrix.shape[0] > 0 else np.zeros_like(target_mean)
