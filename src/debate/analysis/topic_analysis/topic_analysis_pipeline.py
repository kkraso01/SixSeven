...
def load_nrc_emotion_lexicon(path: Optional[str]) -> Dict[str, set]:
    ...
    empty mapping. Preserve that behavior so the shared loader swap does not
    introduce new runtime failures for optional/overridden lexicon paths.
    """
    if not path:
        return {}
...
def main(argv: list[str] | None = None) -> None:
    ...
    emfd_df = shared_load_emfd_lexicon(emfd_lex_path)
