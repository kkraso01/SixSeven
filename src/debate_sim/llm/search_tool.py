...
def generate_conspiracy_query(topic: str, claim: str) -> str:
    """
    Generate a search query for conspiracy-oriented sources.
    
    Args:
        topic: The debate topic
        claim: The specific claim to search for
    
    Returns:
        Search query string targeting fringe/conspiracy sources
    """
    # Target conspiracy forums and alternative news sites
    conspiracy_sites = [
        "site:reddit.com/r/conspiracy",
        "site:reddit.com/r/conspiracytheories",
        "site:beforeitsnews.com",
    ]
    
    # Rotate through sites (use first one for simplicity)
    site_filter = conspiracy_sites[0]
    
    # Combine topic and claim for focused search
    return f"{site_filter} {topic} {claim}".strip()


def generate_scientific_query(topic: str, claim: str) -> str:
    """
    Generate a search query for scientific/mainstream sources.
    
    Args:
        topic: The debate topic
        claim: The specific claim to verify or refute
    
    Returns:
        Search query string targeting scientific/fact-checking sources
    """
    # Target reputable scientific and fact-checking sources
    scientific_sites = [
        "site:cdc.gov",
        "site:nih.gov",
        "site:who.int",
    ...
    # Rotate through sites (use first one for simplicity)
    site_filter = scientific_sites[0]
