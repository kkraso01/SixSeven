...
class DebateConfig:
    ...
    max_rounds_in_history: Optional[int] = None  # Trim to last N rounds
    max_messages_in_history: Optional[int] = None  # Trim to last N messages
    max_chars_in_history: Optional[int] = None  # Trim to last N characters
    ...
    @classmethod
    def from_ini(cls, config_path: str | Path = "config.ini") -> "DebateConfig":
        """Load configuration from an INI file.
        
        Args:
            config_path: Path to the config.ini file. Defaults to "config.ini" in the current directory.
        ...
        def get_optional_int(section: str, key: str) -> Optional[int]:
        ...
        return cls(
            ...
            include_memory_summary=get_bool("history", "include_memory_summary", cls.include_memory_summary),
            ...
            summarize_if_trimmed=get_bool("history", "summarize_if_trimmed", cls.summarize_if_trimmed),
            highlight_opponent_last=get_bool("history", "highlight_opponent_last", cls.highlight_opponent_last),
        )
    
    @classmethod
    def from_env(cls) -> "DebateConfig":
        """Legacy method for backward compatibility. Now uses from_ini() as default."""
        return cls.from_ini()
